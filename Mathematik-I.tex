...
\subsection{Definition Tautologie, Kontradiktion, erfüllbar }\marginpar{21.10.15}
...
\subsubsection{} $\neg (\exists x \in M: P(x))\equiv \forall x\in M:\neg P(x)$\\
	(Verallgemeinerung der Regeln von DeMorgan $\rightarrow$ Bsp. 1.15,4)\vspace{-0.5em}
	\begin{align*}
...
\subsubsection{$A\cup B = A \Leftrightarrow B\subseteq A$}
	...
	$\lbrace x \mid ((x\in X) \wedge (x\not \in A)) \vee ((x \in X) 
	\wedge (x \not \in B)) \rbrace$\\
	\hphantom{\underline{\textsf{d) }} \ $A^C_X \cup B^C_X$\\
	2. Regel analog.
	\underline{\textsf{e) }} \ analog\\
	\underline{\textsf{f), g), h) }} \ später
\section{Beweismethoden}
	Ein mathematischer \underline{\textsf{Beweis}} ist die Herleitung der Wahrheit (oder
	Falschheit) einer Aussage aus einer Menge von \underline{\textsf{Axiomen}} (nicht beweisbare 
	Grundtatsachen) oder bereits bewiesenen Aussagen mittels logischer Folgerungen\\
	Bewiesene Aussagen werden \underline{\textsf{Sätze}} genannt.\\
	\underline{\textsf{Lemma}} \; - \; Hilfssatz, der nur als Grundlage für wichtigeren
	Satz formuliert und bewiesen wird.\\
	\underline{\textsf{Theorem}} \; - \; wichtiger Satz
	\underline{\textsf{Korollar}} \; - \; einfache Folgerung aus Satz, z. B. Spezialfall\\
	\underline{\textsf{Definition}} \; - \; Bennenung/Bestummung eines Begriffs/Symbols.\\
	$\square$ \; - \; Zeichen für Beweisende (auch $\blacksquare , 
	\mathsf{q.e.d.} , wzbw , \dots$)\\
	Mathematische Sätye haben oft die Form: \\
	%TODO Spacing
	wenn $V$ (Voraussetzung) gilt, dann gilt auch $B$ (Behauptung)\\
	($V, \, B$ : Aussagen), \textsf{kurz}: $V\Rightarrow B$\\
...
\subsection{Direkter Beweis}
	...
	[Sei $V$ wahr $\Rightarrow \dots$ \\
	\hphantom{[Sei $V$ wahr }$\Rightarrow \dots$\\
	\hphantom{[Sei $V$ wahr }$\Rightarrow \dots$\\
	\hphantom{[Sei $V$ wahr }$\Rightarrow \dots$\\
	\hphantom{[Sei $V$ wahr }$\Rightarrow \; B$ ist wahr]\\
	\underline{\textsc{Beispiel:}} \; \text{Sei } \\
	$n\in \N\text{. Ist }n\text{ gerade}$, so ist 
	auch $n^2 \text{ gerade.}$\\
	%TODO braces V B
	\begin{align*}
	\Rightarrow & n=2\cdot k \text{ für ein } k\in \N\\
	& (\exists k \in \N \text{ mit } n= 2\cdot k\\
	\Rightarrow & n^2=(2\cdot k)^2 = 4k^2 = 2 \cdot (2k^2)\\
	\Rightarrow n^2 \text{ ist gerade. } \hfill \square 
	\end{align*}
\subsection{Beweis durch Kontraposition}
	...
	[Es gelte $\neq B$ $\Rightarrow \dots$ \\
	\hphantom{[Es gelte $\neq B$ }$\Rightarrow \dots$\\
	\hphantom{[Es gelte $\neq B$ }$\Rightarrow \dots$\\
	\hphantom{[Es gelte $\neq B$ }$\Rightarrow \dots$\\
	\hphantom{[Es gelte $\neq B$ }$\Rightarrow$ Es gilt $\neq V$]\\
	\underline{\textsf{Beispiel:}} Sei $n\in\N,$\\
	$\underbrace{\text{Ist } n^2\text{ gerade}}_V$, so $\underbrace{\text{ist auch } n\text{ gerade}}_B$\\
	\underline{\textsf{Beweis durch Kontraposition:}}\\
	Sei $n$ ungerade \hfill $//\neq B$ gilt.\\
	$\Rightarrow n=2k+1$ für ein $k\in\N_0$\\
...
\subsection{Beweis durch Widerspruch (indirekter Beweis)}
	Zu zeigen ist die Aussage $A$. Wir gehen davon aus, dass $A$ falsch nicht gelte
	($\neq A$ ist wahr) und folgern durch logische Schlüsse eine zweite Aussage $B$,
	von der wir wissen, dass sie falsch ist.\\
	Wenn alle logischen Schlüsse korrekt waren, muss also $\neq A$ falsch gewesen sein,
	also $A$ wahr.\\
	$((\neq A \Rightarrow B)\wedge (\neq B))\Rightarrow A$ ist Tautologie $)$
\end{document}
